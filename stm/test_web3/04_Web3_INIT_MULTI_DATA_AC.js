...
const EXEC_TEST_ACTIONS =
  process.env.NETWORK_ID != 56 && process.env.NETWORK_ID != 1;
...
console.log(
  chalk.red("process.env.WHITELIST_COUNT".padEnd(30, ".")),
  process.env.WHITELIST_COUNT
);
...
describe(`Contract Web3 Interface`, async () => {
  ...
  before(async function () {
    ...
    if (
      (await CONST.web3_call("getContractType", [], nameOverride)) ==
      CONST.contractType.CASHFLOW_CONTROLLER
    ) {
      const baseTypes = (
        await CONST.web3_call("getSecTokenTypes", [], nameOverride)
      ).tokenTypes;
      //console.log('baseTypes', baseTypes);
      console.group();
      for (var baseType of baseTypes) {
        console.log(
          `Whitelisting & sealing (base-type @${baseType.cashflowBaseAddr}): `,
          baseType.name
        );
    ...
    if (EXEC_TEST_ACTIONS) {
      const contractType = await CONST.web3_call(
        "getContractType",
        [],
        nameOverride
      );
      if (contractType == CONST.contractType.COMMODITY) {
        ...
        if (
          !(
            await CONST.web3_call("getSecTokenTypes", [], nameOverride)
          ).tokenTypes.some((p) => p.name == "NEW_TOK_TYPE_A")
        ) {
          await CONST.web3_tx(
            "addSecTokenType",
            [
        ...
        if (
          !(
            await CONST.web3_call("getCcyTypes", [], nameOverride)
          ).ccyTypes.some((p) => p.name == "NEW_CCY_TYPE_A")
        ) {
          await CONST.web3_tx(
            "addCcyType",
            ["NEW_CCY_TYPE_A", "cents", 2],
            OWNER,
            OWNER_privKey,
            nameOverride
          );
  ...
  async function whitelistAndSeal(p) {
    ...
    // WL & seal
    const sealedStatus = await CONST.web3_call(
      "getContractSeal",
      [],
      nameOverride,
      addrOverride
    );
    var allWhitelisted = await CONST.web3_call(
      "getWhitelist",
      [],
      nameOverride,
      addrOverride
    ...
    if (sealedStatus == false) {
      try {
        if (wlMany.length > 0) {
          console.log(
            chalk.inverse(
              `(${addrOverride}) SETUP RESERVED WL (count=${wlMany.length}) last whiteNdx=${whiteNdx}...`
            )
          );
          await CONST.web3_tx(
            "whitelistMany",
    ...
    wlMany = [];
    for (
      whiteNdx = WHITELIST_RESERVED_COUNT;
      whiteNdx < WHITE_MINTER_START_NDX + WHITE_MINTER_COUNT;
      whiteNdx++
    ) {
      x = await CONST.getAccountAndKey(whiteNdx);
      if (
    ...
    if (sealedStatus == false) {
      try {
        if (wlMany.length > 0) {
          console.log(
            chalk.inverse(
              `(${addrOverride}) SETUP MINTERS WL (count=${wlMany.length}) last whiteNdx=${whiteNdx}...`
            )
          );
          await CONST.web3_tx(
            "whitelistMany",
    ...
    for (
      whiteNdx = WHITE_MINTER_START_NDX + WHITE_MINTER_COUNT;
      whiteNdx < WHITE_BUYER_START_NDX + WHITE_BUYER_COUNT;
      whiteNdx++
    ) {
      x = await CONST.getAccountAndKey(whiteNdx);
      if (
    ...
    if (sealedStatus == false) {
      try {
        if (wlMany.length > 0) {
          console.log(
            chalk.inverse(
              `(${addrOverride}) SETUP BUYERS WL (count=${wlMany.length}) last whiteNdx=${whiteNdx}...`
            )
          );
          await CONST.web3_tx(
            "whitelistMany",
    ...
    for (
      whiteNdx = WHITE_BUYER_START_NDX + WHITE_BUYER_COUNT;
      whiteNdx < Number(TEST_ACCOUNT_START_NDX) + Number(TEST_ACCOUNT_COUNT);
      whiteNdx++
    ) {
      x = await CONST.getAccountAndKey(whiteNdx);
      if (
    ...
    if (sealedStatus == false && wlMany.length > 0) {
      console.log(
        chalk.inverse(
          `(${addrOverride}) SETUP TEST ACCOUNTS WL (count=${wlMany.length}) last whiteNdx=${whiteNdx}...`
        )
      );
      await whitelistChunked(wlMany, OWNER, OWNER_privKey);
    }
    ...
    allWhitelisted = await CONST.web3_call(
      "getWhitelist",
      [],
      nameOverride,
      addrOverride
    ...
    const allSubmittedPresent = _.every(submittedToWhitelist, (p) =>
      allWhitelisted.includes(p)
    );
  ...
  it(`web3 direct - UNI (CFT-B) - should be able to mint a uni-batch for an issuer`, async function () {
    ...
    if (
      (await CONST.web3_call("getContractType", [], nameOverride)) !=
      CONST.contractType.CASHFLOW_BASE
    ) {
      this.skip();
      return;
    }
    ...
    if (wei_currentPrice === undefined)
      throw "Undefined ADD_TYPE__ISSUER_wei_currentPrice";
    if (cents_currentPrice === undefined)
      throw "Undefined ADD_TYPE__ISSUER_cents_currentPrice";
    if (qty_saleAllocation === undefined)
      throw "Undefined ADD_TYPE__ISSUER_qty_saleAllocation";
    ...
    const testMeta = [
      {
        ...
        IPFS_PROJECT_DOCUMENT_SALES:
          "QmUwY1VfL5kMxUUdFjmKnbVdaiMYi7UwYVBRWX4A2wom9g",
        IPFS_PROJECT_DOCUMENT_LEGAL:
          "QmWume2gdUoKs3Z8xyDQfPQcZwJHgaKmW9z5ZWDaR4BXNQ",
        IPFS_PROJECT_DOCUMENT_PROSPECTUS:
          "Qmcpmej7yYxoFQ9SGVG6UXP2WU2FHTn9i6sigSdZvsGsLm",
      ...
      },
      {
        ...
        IPFS_PROJECT_DOCUMENT_SALES:
          "QmSeQNDt9rRCeusrnAsW4m7uTDXjso3KtAsw9aBA4bRCnh",
        IPFS_PROJECT_DOCUMENT_LEGAL:
          "QmWume2gdUoKs3Z8xyDQfPQcZwJHgaKmW9z5ZWDaR4BXNQ",
        IPFS_PROJECT_DOCUMENT_PROSPECTUS:
          "Qmcpmej7yYxoFQ9SGVG6UXP2WU2FHTn9i6sigSdZvsGsLm",
      ...
      },
      {
        ...
        IPFS_PROJECT_DOCUMENT_SALES:
          "QmV5AVmqTyCj2Mx1B83N2VggEJVF6jDuq2fJysCCPKSAFk",
        IPFS_PROJECT_DOCUMENT_LEGAL:
          "QmWume2gdUoKs3Z8xyDQfPQcZwJHgaKmW9z5ZWDaR4BXNQ",
        IPFS_PROJECT_DOCUMENT_PROSPECTUS:
          "Qmcpmej7yYxoFQ9SGVG6UXP2WU2FHTn9i6sigSdZvsGsLm",
        URL_PROJECT_IMG:
          "https://www.wilsonco.com/sites/default/files/wilson_final.png",
      },
      {
        ...
        IPFS_PROJECT_DOCUMENT_SALES:
          "QmPxiP4Uz7Stmyb8Vg1UidDUZt4BfGV2LDtCk25zGbUxav",
        IPFS_PROJECT_DOCUMENT_LEGAL:
          "QmeEfjysofDp4H3PrH2fKE65eQMDYBn16Ug1jcG2zW1Ebv",
        IPFS_PROJECT_DOCUMENT_PROSPECTUS:
          "Qmcpmej7yYxoFQ9SGVG6UXP2WU2FHTn9i6sigSdZvsGsLm",
        URL_PROJECT_IMG:
          "https://www.richglory.hk/wp-content/uploads/2019/11/400dpiLogoH.png",
      },
    ];
    var meta;
    switch (contractSymbol) {
      case "WBL1":
        issuerAddr =
          process.env.WEB3_NETWORK_ID == 56
            ? "0xA6F98d2c0e11583877FBc5E06824728E1028400A"
            : "0x07C48Cedb64C4BC0E67191aE26A687BC3EDf2a28";
        meta = testMeta[0];
        break;
      case "SBG1":
        issuerAddr =
          process.env.WEB3_NETWORK_ID == 56
            ? "0x8A56e0C6801B27DB86e111e3Cb652F4494Cb09b5"
            : "0xe59193C3a8c93aA4D17A8cd83fb826F6214B2f77";
        meta = testMeta[1];
        break;
      case "WCO1":
        issuerAddr =
          process.env.WEB3_NETWORK_ID == 56
            ? "0x89ba82d2A05027fF05222C51550590F4df46577F"
            : "0x27a92Bb90CdC9dCFDDf9B4E7FfC80BCF77a43746";
        meta = testMeta[2];
        break;
      case "RG01":
        issuerAddr =
          process.env.WEB3_NETWORK_ID == 56
            ? "0x859C881C1C1D8062Bd0ac46b7a41Fa51140d9f5c"
            : "0xE6b292e9A4d691C17150C8F70046681a3F2B6060";
        meta = testMeta[3];
        break;
      case "RG01B":
        issuerAddr =
      ...
      case "RG01D":
        ...
        break;
      default:
        throw `Unknown contract symbol ${contractSymbol}`;
    }
    ...
    console.log("cfd", cfd);
    if (cfd.issuer == CONST.nullAddr) {
      // not yet issued
      console.log(
        chalk.inverse(
          `Minting uni-batch of ${contractSymbol} for issuer ${issuerAddr}...`
        )
      );
      ...
      const spotTypes = (
        await CONST.web3_call("getSecTokenTypes", [])
      ).tokenTypes.filter((p) => p.settlementType == CONST.settlementType.SPOT);
      console.log("controller spot types", spotTypes);
      const testType = spotTypes.find((p) => p.name == testTypeName);
      if (testType === undefined)
        throw `Failed to find type '${testTypeName}' in controller`;
      ...
      const issuerLedger = await CONST.web3_call("getLedgerEntry", [
        issuerAddr,
      ]);
      ...
      console.log(
        chalk.gray(
          `${contractSymbol} already has uni-batch minted for ${cfd.issuer}; nop.`
        )
      );
    }
    ...
    console.log(
      chalk.inverse(
        `Setting uni-batch issuer values for issuer ${issuerAddr}...`
      )
    );
    ...
    await CONST.web3_tx(
      "setIssuerValues",
      [wei_currentPrice, cents_currentPrice, qty_saleAllocation],
      OWNER,
      OWNER_privKey,
      nameOverride
  ...
  it(`web3 direct - multi - should be able to mint multiple batches for all whitelist minters`, async function () {
    ...
    const curTokTypes = (await CONST.web3_call("getSecTokenTypes", []))
      .tokenTypes;
  ...
  it(`web3 direct - multi - should be able to fund (tokens & ccy), trade & withdraw (tokens & ccy) for all whitelist buyers`, async function () {
    ...
    for (
      var whiteNdx = WHITE_BUYER_START_NDX;
      whiteNdx < WHITE_BUYER_START_NDX + WHITE_BUYER_COUNT;
      whiteNdx++
    ) {
      ...
      for (var buyNdx = 0; buyNdx < BUYS_PER_WHITE_BUYER; buyNdx++) {
        const SELLER =
          WHITE_MINTERS[(whiteNdx + buyNdx) % WHITE_MINTERS.length];
        const minterLedger = await CONST.web3_call("getLedgerEntry", [
          SELLER.addr,
        ]);
        const minterTokTypeId = minterLedger.tokens[0].tokTypeId.toString();

        const exchangeCcyFee = {
          ccy_mirrorFee: false,
          ccy_perMillion: 0,
          fee_fixed: buyNdx * 11,
          fee_percBips: buyNdx * 6,
          fee_min: buyNdx * 11,
          fee_max: buyNdx * 51,
        };
        const ledgerCcyFee = {
          ...
          ccy_perMillion: 0,
          fee_fixed: buyNdx * 12,
          fee_percBips: buyNdx * 7,
          fee_min: buyNdx * 12,
          fee_max: buyNdx * 52,
        };
        await CONST.web3_tx(
          "setFee_CcyType",
          [ccyTypeIdFunded, CONST.nullAddr, exchangeCcyFee],
          OWNER,
          OWNER_privKey
        );
        await CONST.web3_tx(
          "setFee_CcyType",
          [ccyTypeIdFunded, BUYER.addr, ledgerCcyFee],
          OWNER,
          OWNER_privKey
        );

        const exchangeTokFee = {
          ccy_mirrorFee: false,
          ccy_perMillion: 0,
          fee_fixed: buyNdx * 11,
          fee_percBips: buyNdx * 6,
        ...
        await CONST.web3_tx(
          "setFee_TokType",
          [minterTokTypeId, CONST.nullAddr, exchangeTokFee],
          OWNER,
          OWNER_privKey
        );
        await CONST.web3_tx(
          "setFee_TokType",
          [minterTokTypeId, SELLER.addr, ledgerTokFee],
          OWNER,
          OWNER_privKey
        );
        ...
        await CONST.web3_tx(
          ...
          [
            {
              ledger_A: SELLER.addr,
              ledger_B: BUYER.addr,
              qty_A: minterLedger.tokens[0].currentQty.div(2),
              tokTypeId_A: minterTokTypeId,
              qty_B: 0,
              tokTypeId_B: 0,
              ccy_amount_A: 0,
              ccyTypeId_A: 0,
              ccy_amount_B: 5000,
              ccyTypeId_B: ccyTypeIdFunded,
              applyFees: true,
      ...
      await CONST.web3_tx(
        "fundOrWithdraw",
        [
          CONST.fundWithdrawType.WITHDRAW,
          ccyTypeIdFunded,
          100 * (whiteNdx + 1),
          BUYER.addr,
          "TEST_DATA",
      ...
      console.group(
        chalk.inverse(`WITHDRAWING (SELF-CUSTODY) ALL FOR ${BUYER.addr}...`)
      );
          ...
          [
            {
              ledger_A: BUYER.addr,
              ledger_B: GRAY,
              qty_A: buyerLedger.tokens[x].currentQty,
              tokTypeId_A: buyerLedger.tokens[x].tokTypeId.toString(),
              qty_B: 0,
              tokTypeId_B: 0,
              ccy_amount_A: 0,
              ccyTypeId_A: 0,
      ...
      await CONST.web3_tx(
        "transfer",
        [BUYER.addr, buyerLedger.spot_sumQty.toString()],
        GRAY,
        GRAY_privKey
      );
      console.groupEnd();
      ...
      await CONST.web3_tx(
        "setFee_CcyType",
        [CONST.ccyType.USD, CONST.nullAddr, gfUsd],
        OWNER,
        OWNER_privKey
      );
    }
