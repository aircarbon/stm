...
contract("StMaster", accounts => {
    ...
    it('withdrawing - should allow withdrawing of USD', async () => {
        await stm.fund(CONST.ccyType.USD, CONST.thousandUsd_cents * 2, accounts[global.accountNdx], { from: accounts[0] });
    ...
    it('withdrawing - should allow withdrawing of extreme values of USD', async () => {
        await stm.fund(CONST.ccyType.USD, CONST.millionUsd_cents * 1000 * 1000, accounts[global.accountNdx], { from: accounts[0] });
    ...
    it('withdrawing - should allow repeated withdrawing', async () => {
        await stm.fund(CONST.ccyType.USD, 3, accounts[global.accountNdx]);
        for (var i=0 ; i < 3 ; i++) {
    ...
    it('withdrawing - should have reasonable gas cost for withdrawing', async () => {
        ...
        const withdrawTx = await stm.withdraw(CONST.ccyType.USD, CONST.thousandUsd_cents, accounts[global.accountNdx], { from: accounts[0] });
    ...
    it('withdrawing - should allow minting, funding and withdrawing on same ledger entry', async () => {
        ...
        assert(ledgerEntryAfter.ccys.find(p => p.ccyTypeId == CONST.ccyType.USD).balance == CONST.thousandUsd_cents / 2, 'unexpected usd balance in ledger entry after minting, funding & withdrawing');
    });
