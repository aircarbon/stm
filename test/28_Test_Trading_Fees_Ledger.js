...
contract("StMaster", accounts => {
    ...
    it(`fees (ledger) - apply NATURE token ledger override fee 1000 BP + 5 KG fixed (cap 10 KG) on a small trade (fee on A)`, async () => {
        ...
        await stm.setFee_TokType(CONST.tokenType.NATURE, CONST.nullAddr, { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: 0, fee_percBips: 0, fee_min: 0, fee_max: 0 } );
        await stm.setFee_CcyType(CONST.ccyType.ETH, CONST.nullAddr,   { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: 0, fee_percBips: 0, fee_min: 0, fee_max: 0 } );

        // set ledger fee structure NATURE for A: 10% + 5 KG, CAP 10 KG
        const feeBps = 1000; 
        const feeFix = 5;
        const feeCap = 10;
        const setEeuFeeTx = await stm.setFee_TokType(CONST.tokenType.NATURE, A, { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: feeFix, fee_percBips: feeBps, fee_min: 0, fee_max: feeCap } );
        truffleAssert.eventEmitted(setEeuFeeTx, 'SetFeeTokBps', ev => ev.tokenTypeId == CONST.tokenType.NATURE && ev.fee_token_PercBips == feeBps && ev.ledgerOwner == A);
        truffleAssert.eventEmitted(setEeuFeeTx, 'SetFeeTokFix', ev => ev.tokenTypeId == CONST.tokenType.NATURE && ev.fee_tokenQty_Fixed == feeFix && ev.ledgerOwner == A);
        truffleAssert.eventEmitted(setEeuFeeTx, 'SetFeeTokMax', ev => ev.tokenTypeId == CONST.tokenType.NATURE && ev.fee_token_Max == feeCap && ev.ledgerOwner == A);
        assert((await stm.getFee(CONST.getFeeType.TOK, CONST.tokenType.NATURE, A)).fee_fixed == feeFix, 'unexpected carbon fixed fee after setting ledger fee structure');
        assert((await stm.getFee(CONST.getFeeType.TOK, CONST.tokenType.NATURE, A)).fee_percBips == feeBps, 'unexpected carbon bps fee after setting ledger fee structure');
        assert((await stm.getFee(CONST.getFeeType.TOK, CONST.tokenType.NATURE, A)).fee_max == feeCap, 'unexpected carbon max fee after setting ledger fee structure');

        // set different (irrelevant) ledger fee UNFCC for A
        await stm.setFee_TokType(CONST.tokenType.CORSIA, A, { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: feeFix+1, fee_percBips: feeBps+1, fee_min: 0, fee_max: feeCap+1 } );
    ...
    it(`fees (ledger) - apply then clear NATURE token ledger override fee 1000 BP + 5 KG fixed (cap 10 KG) on a small trade (fee on A)`, async () => {
        const A = accounts[global.TaddrNdx + 0];
        const B = accounts[global.TaddrNdx + 1];
        await stm.mintSecTokenBatch(CONST.tokenType.NATURE,    CONST.kt1Carbon, 1,      A, CONST.nullFees, [], [], { from: accounts[0] });
        await stm.fund(CONST.ccyType.ETH,                   CONST.oneEth_wei,        B,                         { from: accounts[0] });
        ...
        await stm.setFee_TokType(CONST.tokenType.NATURE, CONST.nullAddr, { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: globalFeeFix, fee_percBips: globalFeeBps, fee_min: 0, fee_max: globalFeeCap } );
        await stm.setFee_CcyType(CONST.ccyType.ETH, CONST.nullAddr,   { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: 0,            fee_percBips: 0,            fee_min: 0, fee_max: 0 } );
        ...
        var feeFix = 5;
        var feeCap = 10;
        await stm.setFee_TokType(CONST.tokenType.NATURE, A, { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: feeFix, fee_percBips: feeBps, fee_min: 0, fee_max: feeCap } );

        // clear ledger fee NATURE for A (zero)
        feeBps = 0;
        feeFix = 0;
        feeCap = 0;
        const clearEeuFeeTx = await stm.setFee_TokType(CONST.tokenType.NATURE, A, { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: feeFix, fee_percBips: feeBps, fee_min: 0, fee_max: feeCap } );
        truffleAssert.eventEmitted(clearEeuFeeTx, 'SetFeeTokBps', ev => ev.tokenTypeId == CONST.tokenType.NATURE && ev.fee_token_PercBips == feeBps && ev.ledgerOwner == A);
        truffleAssert.eventEmitted(clearEeuFeeTx, 'SetFeeTokFix', ev => ev.tokenTypeId == CONST.tokenType.NATURE && ev.fee_tokenQty_Fixed == feeFix && ev.ledgerOwner == A);
        truffleAssert.eventEmitted(clearEeuFeeTx, 'SetFeeTokMax', ev => ev.tokenTypeId == CONST.tokenType.NATURE && ev.fee_token_Max == feeCap && ev.ledgerOwner == A);
        assert((await stm.getFee(CONST.getFeeType.TOK, CONST.tokenType.NATURE, A)).fee_fixed == feeFix, 'unexpected carbon fixed fee after clearing ledger fee structure');
        assert((await stm.getFee(CONST.getFeeType.TOK, CONST.tokenType.NATURE, A)).fee_percBips == feeBps, 'unexpected carbon bps fee after clearing ledger fee structure');
        assert((await stm.getFee(CONST.getFeeType.TOK, CONST.tokenType.NATURE, A)).fee_max == feeCap, 'unexpected carbon max fee after clearing ledger fee structure');
    ...
    it(`fees (ledger) - apply NATURE token ledger override fee 1000 BP + 1000 KG fixed (collar 100m tons), on a large (0.5 GT) trade (fee on B)`, async () => {
        const A = accounts[global.TaddrNdx + 0];
        const B = accounts[global.TaddrNdx + 1];

        await stm.fund(CONST.ccyType.ETH,                      CONST.oneEth_wei,        A,         { from: accounts[0] });
        await stm.mintSecTokenBatch(CONST.tokenType.NATURE,    CONST.gtCarbon, 1,       B, CONST.nullFees, [], [], { from: accounts[0] });

        // set global fee structure (zero)
        await stm.setFee_TokType(CONST.tokenType.NATURE, CONST.nullAddr, { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: 0, fee_percBips: 0, fee_min: 0, fee_max: 0 } );
        await stm.setFee_CcyType(CONST.ccyType.ETH, CONST.nullAddr,   { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: 0, fee_percBips: 0, fee_min: 0, fee_max: 0 } );

        // set ledger fee structure NATURE on B: 10% + 1000 KG
        const feeBps = 1000; // 1000 bp
        const feeFix = 1000; // 1000 kg
        const feeMin = 100000000000; // 100m tons
        const setEeuFeeTx = await stm.setFee_TokType(CONST.tokenType.NATURE, B, { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: feeFix, fee_percBips: feeBps, fee_min: feeMin, fee_max: 0 } );
        const setCcyFeeTx = await stm.setFee_CcyType(CONST.ccyType.ETH, B,   { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: 0,      fee_percBips: 0,      fee_min: 0,      fee_max: 0 } );
        truffleAssert.eventEmitted(setEeuFeeTx, 'SetFeeTokBps', ev => ev.tokenTypeId == CONST.tokenType.NATURE && ev.fee_token_PercBips == feeBps && ev.ledgerOwner == B);
        truffleAssert.eventEmitted(setEeuFeeTx, 'SetFeeTokFix', ev => ev.tokenTypeId == CONST.tokenType.NATURE && ev.fee_tokenQty_Fixed == feeFix && ev.ledgerOwner == B);
        truffleAssert.eventEmitted(setEeuFeeTx, 'SetFeeTokMin', ev => ev.tokenTypeId == CONST.tokenType.NATURE && ev.fee_token_Min == feeMin && ev.ledgerOwner == B);
        assert((await stm.getFee(CONST.getFeeType.TOK, CONST.tokenType.NATURE, B)).fee_fixed == feeFix, 'unexpected carbon fixed fee after setting ledger fee structure');
        assert((await stm.getFee(CONST.getFeeType.TOK, CONST.tokenType.NATURE, B)).fee_percBips == feeBps, 'unexpected carbon bps fee after setting ledger fee structure');
        assert((await stm.getFee(CONST.getFeeType.TOK, CONST.tokenType.NATURE, B)).fee_min == feeMin, 'unexpected carbon max fee after setting ledger fee structure');

        // set different (irrelevant) ledger fee UNFCC for B
        await stm.setFee_TokType(CONST.tokenType.CORSIA, B, { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: feeFix+1, fee_percBips: feeBps+1, fee_min: feeMin+1, fee_max: 0 } );
        ...
        const data = await transferHelper.transferLedger({ stm, accounts, 
                ledger_A: A,                                   ledger_B: B,
                   ...
                   qty_B: transferAmountKg,               tokenTypeId_B: CONST.tokenType.NATURE,
    ...
    it(`fees (ledger) - apply then clear NATURE token ledger override fee 1000 BP + 1000 KG fixed (collar 100m tons), on a large (0.5 GT) trade (fee on B)`, async () => {
        const A = accounts[global.TaddrNdx + 0];
        const B = accounts[global.TaddrNdx + 1];
        ...
        await stm.setFee_TokType(CONST.tokenType.NATURE, CONST.nullAddr, { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: globalFeeFix, fee_percBips: globalFeeBps, fee_min: globalFeeMin, fee_max: 0 } );
        await stm.setFee_CcyType(CONST.ccyType.ETH, CONST.nullAddr,   { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: 0,            fee_percBips: 0,            fee_min: 0,            fee_max: 0 } );
        ...
        await stm.setFee_TokType(CONST.tokenType.NATURE, B, { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: feeFix, fee_percBips: feeBps, fee_min: feeMin, fee_max: 0 } );
        await stm.setFee_CcyType(CONST.ccyType.ETH, B,   { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: 0,      fee_percBips: 0,      fee_min: 0,      fee_max: 0 } );

        // clear ledger fee structure (zero) on B
        feeBps = 0;
        feeFix = 0;
        feeMin = 0;
        const clearEeuFeeTx = await stm.setFee_TokType(CONST.tokenType.NATURE, B, { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: feeFix, fee_percBips: feeBps, fee_min: feeMin, fee_max: 0 } );
        truffleAssert.eventEmitted(clearEeuFeeTx, 'SetFeeTokBps', ev => ev.tokenTypeId == CONST.tokenType.NATURE && ev.fee_token_PercBips == feeBps && ev.ledgerOwner == B);
        truffleAssert.eventEmitted(clearEeuFeeTx, 'SetFeeTokFix', ev => ev.tokenTypeId == CONST.tokenType.NATURE && ev.fee_tokenQty_Fixed == feeFix && ev.ledgerOwner == B);
        truffleAssert.eventEmitted(clearEeuFeeTx, 'SetFeeTokMin', ev => ev.tokenTypeId == CONST.tokenType.NATURE && ev.fee_token_Min == feeMin && ev.ledgerOwner == B);
        assert((await stm.getFee(CONST.getFeeType.TOK, CONST.tokenType.NATURE, B)).fee_fixed == feeFix, 'unexpected carbon fixed fee after clearing ledger fee structure');
        assert((await stm.getFee(CONST.getFeeType.TOK, CONST.tokenType.NATURE, B)).fee_percBips == feeBps, 'unexpected carbon bps fee after clearing ledger fee structure');
        assert((await stm.getFee(CONST.getFeeType.TOK, CONST.tokenType.NATURE, B)).fee_min == feeMin, 'unexpected carbon max fee after clearing ledger fee structure');
    ...
    it(`fees (ledger) - apply ETH ccy override fee 2500 BP + 0.01 ETH fixed (collar 0.2 ETH), on a small trade (fee on A)`, async () => {
        ...
        const setCcyFeeTx = await stm.setFee_CcyType(CONST.ccyType.ETH, A,   { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: ethFeeFix, fee_percBips: ethFeeBps, fee_min: ethFeeMin, fee_max: 0 } );
    ...
    it(`fees (ledger) - apply then clear ETH ccy override fee 2500 BP + 0.01 ETH fixed (collar 0.2 ETH), on a small trade (fee on A)`, async () => {
        ...
        await stm.setFee_CcyType(CONST.ccyType.ETH, CONST.nullAddr,   { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: globalFeeFix, fee_percBips: globalFeeBps, fee_min: globalFeeMin, fee_max: 0 } );
        await stm.setFee_TokType(CONST.tokenType.NATURE, CONST.nullAddr, { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: 0,            fee_percBips: 0,            fee_min: 0,            fee_max: 0 } );
        ...
        await stm.setFee_CcyType(CONST.ccyType.ETH, A,   { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: ethFeeFix, fee_percBips: ethFeeBps, fee_min: ethFeeMin, fee_max: 0 } );

        // clear ledger fee ETH for A (zero)
        ethFeeBps = 0;
        ethFeeFix = 0;
        ethFeeMin = 0;
        const clearFeeTx = await stm.setFee_CcyType(CONST.ccyType.ETH, A, { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: ethFeeFix, fee_percBips: ethFeeBps, fee_min: ethFeeMin, fee_max: 0 } );
    ...
    it(`fees (ledger) - apply ETH ccy ledger override fee 1000 BP + 1000 ETH fixed (cap 50000 ETH), on a large (500k ETH) trade (fee on B)`, async () => {
        ...
        const setCcyFeeTx = await stm.setFee_CcyType(CONST.ccyType.ETH, B,   { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: ethFeeFix, fee_percBips: ethFeeBps, fee_min: 0, fee_max: ethFeeMax } );
        const setEeuFeeTx = await stm.setFee_TokType(CONST.tokenType.NATURE, B, { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: 0,         fee_percBips: 0,         fee_min: 0, fee_max: 0 } );
    ...
    it(`fees (ledger) - apply then clear ETH ccy ledger override fee 1000 BP + 1000 ETH fixed (cap 50000 ETH), on a large (250k ETH) trade (fee on B)`, async () => {
        ...
        await stm.setFee_CcyType(CONST.ccyType.ETH, CONST.nullAddr,   { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: globalFeeFix, fee_percBips: globalFeeBps, fee_min: 0, fee_max: globalFeeMax } );
        await stm.setFee_TokType(CONST.tokenType.NATURE, CONST.nullAddr, { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: 0,            fee_percBips: 0,            fee_min: 0, fee_max: 0 } );
        ...
        await stm.setFee_CcyType(CONST.ccyType.ETH, B,   { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: ethFeeFix, fee_percBips: ethFeeBps, fee_min: 0, fee_max: ethFeeMax } );
        await stm.setFee_TokType(CONST.tokenType.NATURE, B, { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: 0,         fee_percBips: 0,         fee_min: 0, fee_max: 0 } );

        // clear ledger fee structure (zero) on B
        ethFeeBps = 0;
        ethFeeFix = 0;
        ethFeeMax = 0;
        const clearFeeTx = await stm.setFee_CcyType(CONST.ccyType.ETH, B, { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: ethFeeFix, fee_percBips: ethFeeBps, fee_min: 0, fee_max: ethFeeMax } );
    ...
    it(`fees (ledger) - should allow a ledger fee-capped transfer from A with otherwise insufficient carbon to cover fees (ledger fee on A, global fee on B)`, async () => {
        ...
        await stm.setFee_TokType(CONST.tokenType.NATURE, A, { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: ledgerFeeFix, fee_percBips: ledgerFeeBps, fee_min: 0, fee_max: ledgerFeeMax } );
        ...
        const owner_balBeforeKg = data.owner_before.tokens.filter(p => p.tokenTypeId == CONST.tokenType.NATURE).map(p => p.currentQty).reduce((a,b) => Big(a).plus(Big(b)), Big(0));
        const owner_balAfterKg  =  data.owner_after.tokens.filter(p => p.tokenTypeId == CONST.tokenType.NATURE).map(p => p.currentQty).reduce((a,b) => Big(a).plus(Big(b)), Big(0));
    ...
    it(`fees (ledger) - should allow a ledger fee-capped transfer from B with otherwise insufficient ccy to cover fees (ledger fee on B, global fee on A)`, async () => {
        ...
        await stm.setFee_TokType(CONST.tokenType.NATURE, CONST.nullAddr, { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: globalFeeFix, fee_percBips: globalFeeBps, fee_min: 0, fee_max: globalFeeMax } );

        // set ledger fee structure ETH (B)
        const ledgerFeeFix = 0;
        const ledgerFeeBps = 2000; // 20%
        const ledgerFeeMax = CONST.tenthEth_wei;
        await stm.setFee_CcyType(CONST.ccyType.ETH, B, { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: ledgerFeeFix, fee_percBips: ledgerFeeBps, fee_min: 0, fee_max: ledgerFeeMax } );
    ...
    it(`fees (ledger) - should not allow a transfer with insufficient carbon to cover collared ledger fee (fee on A)`, async () => {
        ...
        const feeFix = 1;
        const feeMin = 101;
        await stm.setFee_TokType(CONST.tokenType.NATURE, A, { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: feeFix, fee_percBips: feeBps, fee_min: feeMin, fee_max: 0 } );
