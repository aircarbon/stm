...
contract("StMaster", accounts => {
    ...
    // ST MULTI FEES
    it(`fees (multi) - apply NATURE token fee 100 BP + 1 KG fixed on a small trade (fee on A)`, async () => {
        ...
        // set fee structure NATURE: 1% + 1 KG
        const feeBps = 100; // 100 bp = 1%
        const feeFix = 1;   // 1 kg
        const setEeuFeeTx = await stm.setFee_TokType(CONST.tokenType.NATURE, CONST.nullAddr, { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: feeFix, fee_percBips: feeBps, fee_min: 0, fee_max: 0 } );
    ...
    it(`fees (multi) - apply NATURE token fee 1000 BP + 1000 KG fixed on a large (0.5 GT) trade (fee on B)`, async () => {
        ...
        // set fee structure NATURE: 10% + 1000 KG
        ...
        // test sender has sent expected quantity and fees
        const ledgerB_VcsKgBefore = data.ledgerB_before.tokens.filter(p => p.tokenTypeId == CONST.tokenType.NATURE).map(p => p.currentQty).reduce((a,b) => Number(a) + Number(b), 0);
        const ledgerB_VcsKgAfter  =  data.ledgerB_after.tokens.filter(p => p.tokenTypeId == CONST.tokenType.NATURE).map(p => p.currentQty).reduce((a,b) => Number(a) + Number(b), 0);
        assert(ledgerB_VcsKgAfter == Number(ledgerB_VcsKgBefore) - Number(expectedFeeKg) - Number(transferAmountKg), 'unexpected ledger A (fee payer) NATURE ST quantity after transfer');
    });
    ...
    it(`fees (multi) - apply ETH ccy fee 100 BP + 0.01 ETH fixed on a small trade (fee on A)`, async () => {
        ...
        const setCcyFeeTx = await stm.setFee_CcyType(CONST.ccyType.ETH, CONST.nullAddr,   { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: ethFeeFix, fee_percBips: ethFeeBps, fee_min: 0, fee_max: 0 } );
    ...
    // CCY + ST MULTI FEES
    it(`fees (multi) - apply ETH ccy fee 1000 BP + 1000 ETH fixed, NATURE fee 1000 BP + 1000 KG on a large (500k ETH / 0.5GT) trade (fees on both sides)`, async () => {
        ...
        // set fee structure NATURE: 10% + 1000 KG fixed
        const eeuFeeBps = 1000; // 1000 bp
        const eeuFeeFix = CONST.kt1Carbon;
        const setEeuFeeTx = await stm.setFee_TokType(CONST.tokenType.NATURE, CONST.nullAddr, { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: eeuFeeFix, fee_percBips: eeuFeeBps, fee_min: 0, fee_max: 0 } );
        truffleAssert.eventEmitted(setEeuFeeTx, 'SetFeeTokBps', ev => ev.tokenTypeId == CONST.tokenType.NATURE && ev.fee_token_PercBips == eeuFeeBps);
        truffleAssert.eventEmitted(setEeuFeeTx, 'SetFeeTokFix', ev => ev.tokenTypeId == CONST.tokenType.NATURE && ev.fee_tokenQty_Fixed == eeuFeeFix);
        ...
        const data = await transferHelper.transferLedger({ stm, accounts, 
                ledger_A: accounts[global.TaddrNdx + 0],                          ledger_B: accounts[global.TaddrNdx + 1],
                   qty_A: 0,                                                   tokenTypeId_A: 0,
                   qty_B: transferAmountEeu,                                   tokenTypeId_B: CONST.tokenType.NATURE,
            ccy_amount_A: transferAmountCcy,                                     ccyTypeId_A: CONST.ccyType.ETH,
            ccy_amount_B: 0,                                                     ccyTypeId_B: 0,
                applyFees: true,
        });
        ...
        const contractOwner_VcsKgBefore = data.owner_before.tokens.filter(p => p.tokenTypeId == CONST.tokenType.NATURE).map(p => p.currentQty).reduce((a,b) => Number(a) + Number(b), 0);
        const contractOwner_VcsKgAfter  =  data.owner_after.tokens.filter(p => p.tokenTypeId == CONST.tokenType.NATURE).map(p => p.currentQty).reduce((a,b) => Number(a) + Number(b), 0);
        assert(contractOwner_VcsKgAfter == Number(contractOwner_VcsKgBefore) + Number(expectedFeeEeu), 'unexpected contract owner (fee receiver) NATURE ST quantity after transfer');
    });
    ...
    it(`fees (multi) - should not allow a transfer with insufficient ccy (fixed + percentage) to cover fees (fee on A)`, async () => {
        ...
        const ethFeeBps = 100; // 100 bp = 1%
        const ethFeeFix = 1;
        await stm.setFee_CcyType(CONST.ccyType.ETH, CONST.nullAddr,   { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: ethFeeFix, fee_percBips: ethFeeBps, fee_min: 0, fee_max: 0 } );
    ...
    it(`fees (multi) - should not allow a transfer with insufficient carbon to cover fees (fee on A)`, async () => {
        ...
        await stm.setFee_TokType(CONST.tokenType.NATURE, CONST.nullAddr, { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: feeFix, fee_percBips: feeBps, fee_min: 0, fee_max: 0 } );
