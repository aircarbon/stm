...
### <span id="IStMintable-addMetaSecTokenBatch-uint64-string-string-"></span> `addMetaSecTokenBatch(uint64 batchId, string metaKeyNew, string metaValueNew)` (external)

Adds a new metadata entry (KVP) to the specified token batch
...
### <span id="IStMintable-getSecToken_countMinted--"></span> `getSecToken_countMinted() → uint256` (external)

Returns the global token count


Tokens are variable-sized, i.e. token count != total quantities in all tokens

### <span id="IStMintable-getSecToken_totalMintedQty--"></span> `getSecToken_totalMintedQty() → uint256` (external)



Returns the sum total quantities in all tokens minted
