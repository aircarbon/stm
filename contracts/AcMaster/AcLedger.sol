...
contract AcLedger is Owned {
    ...
    struct Ledger {
        ...
        mapping(uint256/*EeuType*/ => uint256[]) type_eeuIds;
        mapping(uint256/*EeuType*/ => uint256)   type_sumKG;
    ...
    function getLedgerEntry(address account) external view returns (LedgerReturn memory) {
        ...
        if (_ledger[account].exists) {
            ...
            for (uint256 eeuType = 0; eeuType < uint256(EeuType.dummy_end); eeuType++) {
                countAllEeus += _ledger[account].type_eeuIds[eeuType].length;
            }
            ...
            for (uint256 eeuType = 0; eeuType < uint256(EeuType.dummy_end); eeuType++) {
                uint256[] memory type_eeuIds = _ledger[account].type_eeuIds[eeuType];
