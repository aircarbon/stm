...
describe(`CFT - Base: Issuer Payments Unit Tests`, async () => {
  ...
  it(`Issuer Payments Test 0 (Prep) - Subscribe tokens for CFT from 4 accounts - Base ${NAME_OVERRIDE}`, async () => {
    ...
    try {
      ...
      console.log(
        `Owner 1 (${Owner1.addr}) is now subscribing to ${NAME_OVERRIDE} and paying ${subscriptionAmount1} ETH`
      );
      const subscriptionResponse1 = await CONST.web3_sendEthTestAddr(
        1,
        ADDR_OVERRIDE,
        subscriptionAmount1
      );
      ...
      console.log(
        `Owner 2 (${Owner2.addr}) is now subscribing to ${NAME_OVERRIDE} and paying ${subscriptionAmount2} ETH`
      );
      const subscriptionResponse2 = await CONST.web3_sendEthTestAddr(
        2,
        ADDR_OVERRIDE,
        subscriptionAmount2
      );
      ...
      console.log(
        `Owner 3 (${Owner3.addr}) is now subscribing to ${NAME_OVERRIDE} and paying ${subscriptionAmount3} ETH`
      );
      const subscriptionResponse3 = await CONST.web3_sendEthTestAddr(
        3,
        ADDR_OVERRIDE,
        subscriptionAmount3
      );
      ...
      console.log(
        `Owner 4 (${Owner4.addr}) is now subscribing to ${NAME_OVERRIDE} and paying ${subscriptionAmount4} ETH`
      );
      const subscriptionResponse4 = await CONST.web3_sendEthTestAddr(
        4,
        ADDR_OVERRIDE,
        subscriptionAmount4
      );
  ...
  it(`Issuer Payments Test 2 - should pay first 3 holders from issuer Rich Glory`, async () => {
    ...
    console.log(
      `Processing ${ISSUER_PAYMENT_AMOUNT} issuer payment batch (${BATCH_COUNT})`
    );
    try {
  ...
  it(`Issuer Payments Test 3 - should validate previous payment and make the next payment to remaining 3 token holders - `, async () => {
    ...
    console.log(
      `Processing ${NEXT_ISSUER_PAYMENT_AMOUNT} issuer payment batch (${BATCH_COUNT})`
    );
    try {
      const receiveIssuerPaymentResponse = await CONST.web3_tx(
        "receiveIssuerPaymentBatch", // method to receive payments
    ...
    console.log(
      "Received issuer payment batch: ",
      getIssuerPaymentBatchForSanity
    );
  });
});
