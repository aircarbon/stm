...
describe(`Contract Web3 Interface`, async () => {
  ...
  it(`web3 direct - whitelist many - should be able to write & read a large number of accounts to the whitelist`, async () => {
    ...
    for (var i = 0; i < CHUNKS; i++) {
      console.log(
        `Whitelisting ${CHUNK_SIZE} addresses (chunk ${i} of ${CHUNKS})...`
      );
      const tx = await CONST.web3_tx(
        "whitelistMany",
        [test_accounts.slice(i * CHUNK_SIZE, (i + 1) * CHUNK_SIZE)],
        O.addr,
        O.privKey
      );
      await CONST.logGas(
        CONST.getTestContextWeb3().web3,
        { receipt: tx.receipt },
        "whitelistMany"
      );
    }
