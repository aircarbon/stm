...
describe(`Contract Web3 Interface`, async () => {
  ...
  before(async function () {
    ...
    if (!sealedStatus) {
      const sealTx = await CONST.web3_tx(
        "sealContract",
        [],
        OWNER,
        OWNER_privKey
    ...
    try {
      const mintTx = await CONST.web3_tx(
        "mintSecTokenBatch",
      ...
      const transferTradeTx = await CONST.web3_tx(
        "transferOrTrade",
        [
          {
            ledger_A: WHITE,
