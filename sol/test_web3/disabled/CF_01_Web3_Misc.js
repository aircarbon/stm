...
describe(`Contract Web3 Interface`, async () => {
  ...
  it(`web3 direct - cashflow - balanceOf`, async () => {
    try {
      const data = await CONST.web3_call("balanceOf", [
  ...
  it(`web3 direct - cashflow base types - view direct ledger data of base-types`, async () => {
    ...
    const baseTypes = (await CONST.web3_call("getSecTokenTypes", []))
      .tokenTypes;
    console.log("baseTypes", baseTypes);
    for (var baseType of baseTypes) {
      console.group(baseType.name);
      const ledgerOwners = await CONST.web3_call(
        "getLedgerOwners",
        [],
        undefined /*nameOverride*/,
        baseType.cashflowBaseAddr /*addrOverride*/
      );
      for (var ledgerOwner of ledgerOwners) {
        const ledgerEntry = await CONST.web3_call(
          "getLedgerEntry",
          [ledgerOwner],
          undefined /*nameOverride*/,
          baseType.cashflowBaseAddr /*addrOverride*/
        ...
        for (var token of ledgerEntry.tokens) {
          ...
          tokInfo.push(
            chalk.inverse(
              `tokTypeId:${token.tokTypeId} batchId:${
                token.batchId
              } stId:0x${bn.toString(16)} currentQty:${token.currentQty}`
            )
          );
        }
        console.log(
          `${ledgerOwner} ${ccyInfo.join(" ")}\n\t${tokInfo.join("\n\t")}`
        );
      }
