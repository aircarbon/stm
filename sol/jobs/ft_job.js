...
(async () => {
  ...
  console.log(
    chalk.green.bold.inverse(
      `${"".padStart(16)}${" FUTURES SETTLEMENT ENGINE "}${"".padEnd(16)}`
    )
  );
  console.log();
  ...
  // startup
  console.log(
    `$$$ ${chalk.blue.bgWhite("FT_JOB")} $$$`,
    chalk.dim(process.argv.join(","))
  );
  if (!(await CONST.web3_call("name", [])))
    throw "Failed to validated contract by name. Aborting.";
  if (
    (await CONST.web3_call("getContractType", [])) !=
    CONST.contractType.COMMODITY
  )
    throw "Failed to validated contract type. Aborting.";
  ...
  if (MODE.startsWith("TEST")) {
    ...
    throw "Live mode - TODO";
  } // TODO: live mode: fetch single reference price here, for each future (no participant test data)
  ...
  for (let T = 0; T < MAX_T; T++) {
    ...
    for (let ft of ctx) {
      ...
      for (let p of ft.data.TEST_PARTICIPANTS) {
        ...
        console.log(
          chalk.blue.italic(`PID=${p.id}`) +
            chalk.dim(
              ` ${le.tokens
                .map(
                  (p2) =>
                    `{ #${p2.stId}/Q:${p2.mintedQty.toString().padStart(3)} }`
                )
                .join(", ")}`
            )
        ); //, le.tokens);
      ...
      await TakePay_v2(
        ft.ftId,
        MP,
        ft.data.TEST_PARTICIPANTS.map((p) => p.account)
      );
      console.groupEnd();
    }
  ...
  for (let ft of ctx) {
    ...
    ).tokenTypes.find((p) => p.id == ft.ftId);
    data.push(["Account", "Before", "After", "Deposits", "Withdraws", "Net"]);
    for (let p of ft.data.TEST_PARTICIPANTS) {
      p.le_after = await CONST.web3_call("getLedgerEntry", [p.account]);
      const ccyBefore = Number(
        p.le_before.ccys.find((p) => p.ccyTypeId.eq(ftData.ft.refCcyId)).balance
      );
      const ccyAfter = Number(
        p.le_after.ccys.find((p) => p.ccyTypeId.eq(ftData.ft.refCcyId)).balance
      );
      const ccyDeposits = Number(
        p.ccy_deposits
          ? p.ccy_deposits
              .map((p) => p.a)
    ...
    data.push([
      ...
      "-",
      "-",
      `$${nets.reduce((a, b) => a + b, 0).toString()}`,
    ]);
...
async function processTestContext(ft, T) {
  ...
  for (let p of TEST_PARTICIPANTS.filter((p) => p.id > 0)) {
    ...
    if (ccy_deposit.a) {
      console.log(
        chalk.blue.italic(
          `TEST_PARTICIPANT: PID=${p.id}, account=${p.account}`
        ) + chalk.blue(` ** DEPOSIT ** `),
        ccy_deposit
      );
      await CONST.web3_tx(
  ...
  for (let p of TEST_PARTICIPANTS.filter((p) => p.id > 0)) {
    ...
    if (ft_long.q) {
      console.log(
        chalk.blue.italic(
          `TEST_PARTICIPANT: PID=${p.id}, account=${p.account}`
        ) + chalk.blue(` ** OPEN FUTURES POSITION ** `),
        ft_long
      );
      const shortAccount = TEST_PARTICIPANTS.find(
        (p) => p.id == ft_long.cid
      ).account;

      const { txHash, receipt, evs } = await CONST.web3_tx(
        "openFtPos",
        [
          {
            tokTypeId: ft.ftId,
            ledger_A: p.account,
            ledger_B: shortAccount,
            qty_A: ft_long.q,
            qty_B: ft_long.q * -1,
            price: ft_long.p,
          },
      ...
      const shortStId =
        Number(await CONST.web3_call("getSecToken_MaxId", [])) - 1;
...
async function initTestMode(testMode) {
  ...
  var fts = (await CONST.web3_call("getSecTokenTypes", [])).tokenTypes.filter(
    (p) => p.settlementType == CONST.settlementType.FUTURE
  );
  ...
  fts = (await CONST.web3_call("getSecTokenTypes", [])).tokenTypes.filter(
    (p) => p.settlementType == CONST.settlementType.FUTURE
  );
  ...
  const freshAccounts = _.differenceWith(
    accounts,
    ledgerOwners,
    (a, b) => a.toLowerCase() == b.toLowerCase()
  );
  if (freshAccounts.length == 0)
    throw "Insufficient fresh test accounts: redeploy the contract";
  ...
  var i = 0,
    ret;
  if (testMode == "TEST_0") {
    // single position - desc price
    ret = [
      {
        ftId: fts.find((p) => p.name == TEST_FT_1).id.toString(),
        data: {
          // ## TODO: fails w/ "Central insufficient for settlement" becuase not ordering TP2 joib by OTM first?!
          price: [100, 99, 98, 97, 96, 95, 94, 93, 92],
          TEST_PARTICIPANTS: [
            {
              id: 1,
              account: freshAccounts[i++],
  ...
  } else if (testMode == "TEST_1") {
    // single position - asc price
    ret = [
      {
        ftId: fts.find((p) => p.name == TEST_FT_1).id.toString(),
        data: {
    ...
    // adding to positions, 2 parties (pos-combine)
    ret = [
      {
        ftId: fts.find((p) => p.name == TEST_FT_1).id.toString(),
        data: {
    ...
    // A - B - C (counterparty gets swapped)
    ret = [
      {
        ftId: fts.find((p) => p.name == TEST_FT_1).id.toString(),
        data: {
    ...
    // margin call -> zero (## settelment fails, by design ##)
    ret = [
      {
        ftId: fts.find((p) => p.name == TEST_FT_1).id.toString(),
        data: {
