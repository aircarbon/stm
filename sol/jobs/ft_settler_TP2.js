...
module.exports = {
  TakePay_v2: async (ftId, MP, test_ledgerOwners) => {
    ...
    const ft = (await CONST.web3_call("getSecTokenTypes", [])).tokenTypes.find(
      (p) => p.id == ftId
    );
    ...
    for (let addr of ledgerOwners) {
      ...
      if (includeAccount) {
        const shorts = le.tokens.filter(
          (p) => p.tokTypeId == ftId && p.currentQty.lt(0)
        );
        const longs = le.tokens.filter(
          (p) => p.tokTypeId == ftId && p.currentQty.gt(0)
        );
        const closed = le.tokens.filter(
          (p) => p.tokTypeId == ftId && p.currentQty.eq(0)
        );
...
async function runTakePay(O, ft, ftId, posId, MP, FEE_PER_SIDE, SIDE) {
  ...
  const to_desc =
    ev.to.toLowerCase() == O.addr.toLowerCase() ? "[C]" : `[${SIDE}]`;
  const from_desc =
    ev.from.toLowerCase() == O.addr.toLowerCase() ? "[C]" : `[${SIDE}]`;
  ...
  const ccy = le.ccys.find((p) => p.ccyTypeId.eq(ft.ft.refCcyId)),
    bal = Number(ccy.balance.toString()),
    res = Number(ccy.reserved.toString());
  ...
  const resStr = `[ B:$${bal.toFixed(0).padStart(5)} / R:$${res
    .toFixed(0)
    .padStart(5)} ${chartStr} ]`; // 👎 // ✋ // 👌
  ...
  // log TP info
  console.log(
    `${chalk.dim(`${SIDE} stId:${posId.padEnd(3)}`)}` +
      (le_pos.ft_PL.gt(0)
        ? chalk.greenBright(plStr)
        : le_pos.ft_PL.lt(0)
        ? chalk.redBright(plStr)
        : chalk.gray(plStr)) +
      ` > ` +
      chalk.greenBright(`\
${chalk.inverse(from_desc)} ${chalk.dim(truncMiddle(ev.from, 8))} ($${Number(
        from_le.ccys
          .find((p) => p.ccyTypeId.eq(ft.ft.refCcyId))
          .balance.toString()
      )
        .toFixed(0)
        .padEnd(8)}) ==> \
[$${Number(ev.done.toString()).toFixed(0).padStart(7)} `) +
      ...
      chalk.greenBright(`)] ==> \
${chalk.inverse(to_desc)} ${chalk.dim(truncMiddle(ev.to, 8))} ($${Number(
        to_le.ccys
          .find((p) => p.ccyTypeId.eq(ft.ft.refCcyId))
          .balance.toString()
      )
