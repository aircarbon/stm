...
describe(`Cashflow Tokens Test Descriptions and Cases: `, async () => {
  ...
  describe(`New cashflow token deployment, minting and initiate issuance values: `, async () => {
    before(async function (deployer) {
      ...
      expect(
        cfd.issuer.toLowerCase() === ISSUER_RG.addr.toLowerCase(),
        "Issuer expected to be Rich Glory <0xE6b292e9A4d691C17150C8F70046681a3F2B6060>"
      );

      console.log(
        "Token Price : ",
        formatter.hex2eth(cfd.wei_currentPrice),
        " ETH"
      );
      expect(
        formatter.hex2eth(cfd.wei_currentPrice) === 0.1,
        "Expecting Token Price to be 0.1 ETH"
      );
      console.log("Max Issuance : ", formatter.hex2int(cfd.qty_issuanceMax));
      expect(
        formatter.hex2int(cfd.qty_issuanceMax) === 10000,
        "Expecting Max Issuance to be 10000"
      );
      console.log(
        "Sale Allocation : ",
        formatter.hex2int(cfd.qty_saleAllocation)
      );
      expect(
        formatter.hex2int(cfd.qty_saleAllocation) === 1000,
        "Expecting Sale Allocation to be 1000"
      );
    });
    ...
    it(`Issuer Payments Test 0 (Prep) - Subscribe to (Purchase) an issuance from 4 test accounts - Base ${nameOverride}`, async () => {
      ...
      try {
        ...
        console.log(
          `Owner 1 (${Owner1.addr}) is now subscribing to ${nameOverride} and paying ${subscriptionAmount1} ETH`
        );
        const subscriptionResponse1 = await web3_sendEthTestAddr(
          1,
          ADDR_OVERRIDE,
          subscriptionAmount1
        );
        ...
        console.log(
          `Owner 2 (${Owner2.addr}) is now subscribing to ${nameOverride} and paying ${subscriptionAmount2} ETH`
        );
        const subscriptionResponse2 = await web3_sendEthTestAddr(
          2,
          ADDR_OVERRIDE,
          subscriptionAmount2
        );
        ...
        console.log(
          `Owner 3 (${Owner3.addr}) is now subscribing to ${nameOverride} and paying ${subscriptionAmount3} ETH`
        );
        const subscriptionResponse3 = await web3_sendEthTestAddr(
          3,
          ADDR_OVERRIDE,
          subscriptionAmount3
        );
        ...
        console.log(
          `Owner 4 (${Owner4.addr}) is now subscribing to ${nameOverride} and paying ${subscriptionAmount4} ETH`
        );
        const subscriptionResponse4 = await web3_sendEthTestAddr(
          4,
          ADDR_OVERRIDE,
          subscriptionAmount4
        );
        ...
        console.log(
          "Issuance Remaining : ",
          formatter.hex2int(cfd.qty_issuanceRemaining)
        );
        expect(
          formatter.hex2int(cfd.qty_issuanceRemaining) === 9900,
          "Expecting issuance remaining to be 9900"
        );
        console.log(
          "Issuance Sold : ",
          formatter.hex2int(cfd.qty_issuanceSold)
        );
        expect(
          formatter.hex2int(cfd.qty_issuanceSold) === 100,
          "Expecting issuance sold to be 100"
        );
      } catch (ex) {
        console.error(ex);
      }
    ...
    it(`Issuer Payments Test 1 (Pre-processing Test) - should have initial issuer payment values as 0x00`, async () => {
      ...
      console.log(
        "Received issuer payment batch: ",
        getIssuerPaymentByPaymentIdResponse
      );
    });

    it(`Issuer Payments Test 2 - should pay first ${BATCH_COUNT} holders from issuer Rich Glory`, async () => {
      console.log(
        `Processing ${ISSUER_PAYMENT_AMOUNT} issuer payment batch (${BATCH_COUNT}) for Payment ID: #${PAYMENT_ID}`
      );
      try {
        const ISSUER_RG = await getAccountAndKey(13); // Rich Glory '0xE6b292e9A4d691C17150C8F70046681a3F2B6060'
        ...
        console.log(
          "Issuer Payment process response: ",
          receiveIssuerPaymentResponse
        );
      } catch (ex) {
        console.error(ex);
      }
    ...
    it(`Issuer Payments Test 3 - should validate previous payment and make the next ${BATCH_COUNT} token holders - `, async () => {
      ...
      console.log(
        `Processing ${NEXT_ISSUER_PAYMENT_AMOUNT} issuer payment batch (${BATCH_COUNT}) for Payment ID: #${PAYMENT_ID}`
      );
      try {
        const ISSUER_RG = await getAccountAndKey(13); // Rich Glory '0xE6b292e9A4d691C17150C8F70046681a3F2B6060'
        const receiveIssuerPaymentResponse = await web3_tx(
      ...
      console.log(
        "Received issuer payment batch (Sanity Check): ",
        getIssuerPaymentSanity
      );
    });
  });
});
