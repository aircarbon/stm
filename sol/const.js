...
console.log(
  chalk.red("process.env.INSTANCE_ID".padEnd(30, ".")),
...
const WEB3_GWEI_GAS_BID = process.env.INSTANCE_ID.includes("_56")
  ? "20" // BSC mainnet
  : process.env.INSTANCE_ID.includes("_97")
  ? "20" // BSC testnet
  : process.env.INSTANCE_ID === "PROD_52101"
  ? "1" // AC privnet
  : process.env.INSTANCE_ID === "PROD_1"
  ? "80" // ETH mainnet
  : "5";
...
const blocksFromSecs = (secs) =>
  Math.ceil(
    secs /
      (process.env.NETWORK_ID == 97 || process.env.NETWORK_ID == 56 ? 3 : 15)
  );
...
module.exports = {
  ...
  chainlinkAggregators: {
    ...
    42101: {
      // AC sidechain testnet
      btcUsd: "0x0000000000000000000000000000000000000000",
      ethUsd: "0x0000000000000000000000000000000000000000",
      bnbUsd: "0x0000000000000000000000000000000000000000",
    },
    52101: {
      // AC sidechain prodnet
      btcUsd: "0x0000000000000000000000000000000000000000",
      ethUsd: "0x0000000000000000000000000000000000000000",
      bnbUsd: "0x0000000000000000000000000000000000000000",
    },
    56: {
      // BSC mainnet - https://docs.binance.org/smart-chain/developer/link.html
      btcUsd: "0x0000000000000000000000000000000000000000",
      ethUsd: "0x0000000000000000000000000000000000000000",
      bnbUsd: "0x0567F2323251f0Aab15c8dFb1967E4e8A7D42aeE",
    },
    97: {
      // BSC testnet...? https://docs.chain.link/docs/reference-data-contracts-binance-smart-chain#price-feeds
      btcUsd: "0x0000000000000000000000000000000000000000",
      ethUsd: "0x0000000000000000000000000000000000000000",
  ...
  getAccountAndKey: async (accountNdx, mnemonic, coinTypeSlip44) =>
    getAccountAndKey(accountNdx, mnemonic, coinTypeSlip44),

  web3_sendEthTestAddr: (sendFromNdx, sendToAddr, ethValue) =>
    web3_sendEthTestAddr(sendFromNdx, sendToAddr, ethValue),
  ...
  millionEth_wei: Big(web3.utils.toWei("1", "ether"))
    .times(1000)
    .times(1000)
    .toFixed(), // "1000000000000000000000000",
  ...
  logGas: async (truffleWeb3, truffleTx, desc) => {
    // actual gas price, not estimated
    //console.log('truffleTx', truffleTx);

    const web3Tx = await truffleWeb3.eth.getTransaction(
      truffleTx.receipt.transactionHash
    );
    ...
    console.log(`>>> gasUsed - [${process.env.INSTANCE_ID}] ${chalk.inverse(
      desc
    )}: \
${truffleTx.receipt.gasUsed} gas * ${web3.utils.fromWei(
      web3Tx.gasPrice,
      "gwei"
    )}\
...
function getTestContextWeb3(useWs) {
  const context =
    // dev - DM
    process.env.WEB3_NETWORK_ID == 888
      ? { web3: new Web3("http://127.0.0.1:8545"), ethereumTxChain: {} }
      : // dev - Dung
      process.env.WEB3_NETWORK_ID == 889
      ? { web3: new Web3("http://127.0.0.1:8545"), ethereumTxChain: {} }
      : // dev - Vince
      process.env.WEB3_NETWORK_ID == 890
      ? { web3: new Web3("http://127.0.0.1:8545"), ethereumTxChain: {} }
      : // dev - Ankur
      process.env.WEB3_NETWORK_ID == 891
      ? { web3: new Web3("http://127.0.0.1:8545"), ethereumTxChain: {} }
      : // Ropsten - AC Geth
      process.env.WEB3_NETWORK_ID == 3
      ? {
          web3: new Web3("https://ac-dev0.net:9545"),
          ethereumTxChain: { chain: "ropsten", hardfork: "petersburg" },
        }
      ...
      process.env.WEB3_NETWORK_ID == 4
      ? {
          web3: new Web3(
            "https://rinkeby.infura.io/v3/05a8b81beb9a41008f74864b5b1ed544"
          ),
          ethereumTxChain: { chain: "rinkeby", hardfork: "petersburg" },
        }
      : // Sidechain Testnet - AC Geth
      process.env.WEB3_NETWORK_ID == 42101
      ? {
          web3: new Web3(
            useWs ? "wss://ac-dev1.net:9546" : "https://ac-dev1.net:9545"
          ),
          ethereumTxChain: {
      ...
      process.env.WEB3_NETWORK_ID == 52101
      ? {
          web3: new Web3(
            useWs
              ? "wss://ac-prod0.aircarbon.co:9546"
      ...
      process.env.WEB3_NETWORK_ID == 97
      ? {
          web3: new Web3(
            useWs
              ? "wss://ac-prod1.aircarbon.co:8546"
              : "https://ac-prod1.aircarbon.co:8545"
          ),
          ethereumTxChain: {
            common: EthereumJsCommon.forCustomChain(
              "ropsten", // forCustomChain() requires a "known" name!?
      ...
      : // Matic Mainnet
      process.env.WEB3_NETWORK_ID == 137
      ? {
          web3: new Web3(
            useWs
      ...
      : // Matic (Mumbai) Testnet
      process.env.WEB3_NETWORK_ID == 80001
      ? {
          web3: new Web3(
            useWs
      ...
      process.env.WEB3_NETWORK_ID == 1
      ? {
          web3: new Web3(
            "https://mainnet.infura.io/v3/25a36609b48744bdaa0639e7c2b008d9"
          ),
          ethereumTxChain: {},
        }
      : undefined;
...
async function getAccountAndKey(accountNdx, mnemonic, coinTypeSlip44) {
  ...
  const addr_node = hdk.derivePath(
    `m/44'/${coinTypeSlip44 || "60"}'/0'/0/${accountNdx}`
  );
...
async function web3_call(
  ...
  if (addrOverride == undefined) {
    const contractName =
      process.env.CONTRACT_PREFIX +
      (nameOverride || contractProps[process.env.CONTRACT_TYPE].contractName);
    const contractVer =
      process.env.CONTRACT_VER ||
      contractProps[process.env.CONTRACT_TYPE].contractVer;
    contractDb = (
      await db.GetDeployment(
        process.env.WEB3_NETWORK_ID,
        contractName,
        contractVer
      )
    ).recordset[0];
    if (!contractDb)
      throw Error(
        `Failed to lookup contract deployment for (nameOverride=[${nameOverride}]): networkId=${process.env.WEB3_NETWORK_ID}, contractName=${contractName}, contractVer=${contractVer} from ${process.env.sql_server}`
      );
  } else {
    contractDb = (
      await db.GetDeploymentByAddress(process.env.WEB3_NETWORK_ID, addrOverride)
    ).recordset[0];
    if (!contractDb)
      throw Error(
        `Failed to lookup contract deployment for (addrOverride=[${addrOverride}]): networkId=${process.env.WEB3_NETWORK_ID} from ${process.env.sql_server}`
      );
  }
  if (consoleOutput)
    console.log(
      chalk.dim(
        ` > CALL: [${chalk.greenBright(contractDb.contract_enum)} ${
          contractDb.contract_ver
        } @${contractDb.addr}] ${chalk.reset.blue.bgWhite(
          methodName +
            "(" +
            methodArgs.map((p) => JSON.stringify(p)).join() +
            ")"
        )}` +
          chalk.dim(
            ` [networkId: ${process.env.WEB3_NETWORK_ID} - ${web3.currentProvider.host}] - ${process.env.sql_server}`
          )
      )
    );
  var contract = new web3.eth.Contract(
    JSON.parse(contractDb.abi),
    contractDb.addr
  );
  if ((await contract.methods["version"]().call()) != contractDb.contract_ver)
    throw "Deployed contract missing or version mismatch"; // test contract exists - will silently return null on calls if it's not deployed, wtf
  if (fromAddr) {
...
async function web3_tx(
  ...
  const nonce =
    WEB3_NONCE_REPLACE ||
    (await web3.eth.getTransactionCount(fromAddr, "pending"));
  if (consoleOutput)
    console.log(
      chalk.dim(
        ` >   TX: [${chalk.greenBright(
          contractDb.contract_enum
        )} nonce=${nonce} ${contractDb.contract_ver} @${
          contractDb.addr
        }] ${chalk.reset.red.bgWhiteBright(
          methodName +
            "(" +
            methodArgs.map((p) => JSON.stringify(p)).join() +
            ")"
          ...
          chalk.dim(
            `     ... (from: ${fromAddr} / gwei: ${WEB3_GWEI_GAS_BID} / networkId: ${process.env.WEB3_NETWORK_ID} / node: ${web3.currentProvider.host} / db: ${process.env.sql_server})`
          )
      )
    );
  var paramsData = contract.methods[methodName](...methodArgs).encodeABI();
  const txData = {
    ...
    web3.eth
      ...
      .once("confirmation", async (confirms, receipt) => {
        ...
        if (consoleOutput)
          console.log(
            chalk.dim.yellow(
              `   => ${txHash} - ${confirms} confirm(s), secs=${(
                msElapsed / 1000
              ).toFixed(1)} receipt.gasUsed=${
                receipt.gasUsed
              } receipt.blockNumber=${receipt.blockNumber} evs=${evs
                .map((p) => `B# ${p.blockNumber}: ${p.event}`)
                .join(",")}`
            )
...
async function web3_sendEthTestAddr(sendFromNdx, sendToAddr, ethValue) {
  const { addr: fromAddr, privKey: fromPrivKey } = await getAccountAndKey(
    sendFromNdx
  );
  ...
  if (consoleOutput)
    console.log(
      ` > TX web3_sendEthTestAddr: Ξ${chalk.red.bgWhite(
        ethValue.toString()
      )} @ ${chalk.red.bgWhite(fromAddr)} => ${chalk.red.bgWhite(
        sendToAddr
      )} [networkId: ${process.env.WEB3_NETWORK_ID} - ${
    ...
    web3.eth
      ...
      .once("confirmation", async (confirms) => {
        ...
        if (consoleOutput)
          console.log(
            chalk.yellow(
              `   => ${txHash} - ${confirms} confirm(s), receipt.gasUsed=`,
              receipt.gasUsed
            )
          );
        resolve(txHash);
