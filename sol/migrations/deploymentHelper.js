  ...
  Deploy: async (p) => {
    try {
      ...
      if (
        ...
        throw "Unknown contractType";

      if (custodyType != "SELF_CUSTODY" && custodyType != "THIRD_PARTY_CUSTODY")
        throw "Unknown custodyType";
                      ...
                      await deployer
                        ...
                        .then(async (transferLib) => {
                          ...
                          await deployer
                                ...
                                .then(async (feeLib) => {
                                  ...
                                  await deployer
                                    ...
                                    .then(async (loadLib) => {
                                      ...
                                      await deployer
                                        .deploy(PayableLib)
                                        .then(async (payableLib) => {
                                          deployer.link(PayableLib, StMaster);

                                          await deployer
                                            .deploy(FuturesLib)
                                            .then(async (futuresLib) => {
                                              deployer.link(
                                                FuturesLib,
                                                StMaster
                                              );
                                              ...
                                              const MNEMONIC =
                                                ...
                                                require("../DEV_MNEMONIC.js")
                                                  .MNEMONIC;
                                              const accountAndKeys = [];
                                              for (
                                                let i = 0;
                                                i <
                                                CONST.RESERVED_ADDRESSES_COUNT;
                                                i++
                                              ) {
                                                accountAndKeys.push(
                                                  await CONST.getAccountAndKey(
                                                    i,
                                                    MNEMONIC
                                                  )
                                                );
                                              }
                                              const owners = accountAndKeys.map(
                                                (p) => p.addr
                                              );
                                              if (
                                                contractType == "CASHFLOW_BASE"
                                                ...
                                                if (!controllerContract) {
                                                  console.log(
                                                    chalk.bold.red(
                                                      `Failed to lookup controller contract: networkId=${process.env.WEB3_NETWORK_ID}, contractName=${controllerName}, contractVer=${baseVer} from ${process.env.sql_server}`
                                                    )
                                                  );
                                                  process.exit(1);
                                                }
                                                ...
                                                .then(async (stm) => {
                                                  ...
                                                  if (
                                                    ...
                                                    publicIp
                                                      ...
                                                      .catch((e) => {
                                                        console.log(
                                                          "\tWARN: could not get IP - will write 'unknown'"
                                                        );
                                                      });
                                                    console.log(
                                                      `>>> SAVING DEPLOYMENT: ${contractName} ${CONST.contractProps[contractType].contractVer} to ${process.env.sql_server}`
                                                    );
                                                    ...
                                                    const contractOwners =
                                                      await CONST.web3_call(
                                                        "getOwners",
                                                        [],
                                                        undefined /*nameOverride*/,
                                                        stm.address /*addrOverride*/
                                                      );
                                                    if (
                                                      contractType !=
                                                        "CASHFLOW_BASE" &&
                                                      ...
                                                      console.log(
                                                        chalk.red.bold.inverse(
                                                          `Deployment failed: unexpected owners data`
                                                        ),
                                                        contractOwners
                                                      );
                                                      process.exit(1);
                            ...
                            .catch((err) => {
                              console.error(
                                "failed deployment: SpotFeeLib",
                                err
                              );
                            });
                        })
                        .catch((err) => {
                          console.error("failed deployment: TransferLib", err);
                        });
                    })
                    .catch((err) => {
                      console.error("failed deployment: TokenLib", err);
                    });
                })
                .catch((err) => {
                  console.error("failed deployment: CcyLib", err);
                });
            })
            .catch((err) => {
              console.error("failed deployment: LedgerLib", err);
            });
        })
        .catch((err) => {
          console.error("failed deployment: StructLib", err);
        });
      return stmAddr;
    } catch (err) {
      assert;
...
function logEnv(phase, owners, contractType, contractName) {
  ...
  console.log(
    chalk.red("\t process.env.CONTRACT_PREFIX: "),
    process.env.CONTRACT_PREFIX
  );
  console.log(
    chalk.red("\t      process.env.NETWORK_ID: "),
