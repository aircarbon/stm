...
module.exports = async function (deployer) {
  ...
  switch (process.env.INSTANCE_ID) {
    case undefined:
      console.log(
        `Deploying localhost contract instance, saving to DB: ${chalk.inverse(
    ...
    case "DEV":
      console.log(
        `Deploying (AWS DEV / DEV) instance, saving to DB: ${chalk.inverse(
          process.env.sql_server
        )}`
      );
      break;
    case "DEMO":
      console.log(
        `Deploying (AWS DEV / DEMO) instance, saving to DB: ${chalk.inverse(
          process.env.sql_server
        )}`
      );
      break;
    ...
    case "UAT_80001_AC":
      console.log(
        `Deploying (AWS UAT / Matic Testnet (Mumbai) instance, saving to DB: ${chalk.inverse(
          process.env.sql_server
        )}`
      );
      break;
    case "UAT":
      console.log(
        `Deploying (AWS DEV / UAT) instance, saving to DB: ${chalk.inverse(
          process.env.sql_server
        )}`
      );
      break;
    case "PROD_1":
      console.log(
        `Deploying (AWS PROD / ETH 1 MAINNET) instance, saving to DB: ${chalk.inverse(
          process.env.sql_server
        )}`
      );
      break;
    case "PROD_52101":
      console.log(
        `Deploying (AWS PROD / AC 52101 PRODNET) instance, saving to DB: ${chalk.inverse(
          process.env.sql_server
        )}`
      );
      break;
    case "PROD_56":
      console.log(
        `Deploying (AWS PROD / BSC 56 MAINNET) instance, saving to DB: ${chalk.inverse(
          process.env.sql_server
        )}`
      );
      break;
    ...
    case "DEV_SD":
      console.log(
        `Deploying (AWS DEV / DEV [controller w/ 0 default base types] for SDAX) instance, saving to DB: ${chalk.inverse(
          process.env.sql_server
        )}`
      );
      break;
    case "UAT_SD":
      console.log(
        `Deploying (AWS DEV / UAT [controller w/ 0 default base types] for SDAX) instance, saving to DB: ${chalk.inverse(
          process.env.sql_server
        )}`
      );
      break;
    case "UAT_97_SD":
      console.log(
        `Deploying (AWS UAT / BSC Testnet [controller w/ 0 default base types] for SDAX) instance, saving to DB: ${chalk.inverse(
          process.env.sql_server
        )}`
      );
      break;
    ...
    case "UAT_80001_SD":
      console.log(
        `Deploying (AWS UAT / Matic Testnet (Mumbai) [controller w/ 0 default base types] for SDAX) instance, saving to DB: ${chalk.inverse(
          process.env.sql_server
        )}`
      );
      break;
    case "DEMO_97_SD":
      console.log(
        `Deploying (AWS DEMO / BSC Testnet [controller w/ 0 default base types] for SDAX) instance, saving to DB: ${chalk.inverse(
          process.env.sql_server
        )}`
      );
      break;
    case "PROD_3_SD":
      console.log(
        `Deploying (AWS PROD / Ropsten 3 [controller w/ 0 default base types] for SDAX) instance, saving to DB: ${chalk.inverse(
          process.env.sql_server
        )}`
      );
      break;
    case "PROD_56_SD":
      console.log(
        `Deploying (AWS PROD / BSC Mainnet [controller w/ 0 default base types] for SDAX) instance, saving to DB: ${chalk.inverse(
          process.env.sql_server
        )}`
      );
      break;
    case "PROD_137_SD":
      console.log(
        `Deploying (AWS PROD / Matic Mainnet [controller w/ 0 default base types] for SDAX) instance, saving to DB: ${chalk.inverse(
          process.env.sql_server
        )}`
      );
      break;
    ...
    default:
      if (process.env.INSTANCE_ID.startsWith("UAT_3_SD_")) {
        console.log(
          `Deploying (AWS DEV / UAT Ropsten [additional base type] for SDAX) instance, saving to DB: ${chalk.inverse(
            process.env.sql_server
          )}`
        );
      } else if (process.env.INSTANCE_ID.startsWith("UAT_97_SD_")) {
        console.log(
          `Deploying (AWS DEV / UAT BSC Testnet [additional base type] for SDAX) instance, saving to DB: ${chalk.inverse(
            process.env.sql_server
          )}`
        );
      } else if (process.env.INSTANCE_ID.startsWith("UAT_80001_SD_")) {
        console.log(
          `Deploying (AWS DEV / UAT Matic Testnet (Mumbai) [additional base type] for SDAX) instance, saving to DB: ${chalk.inverse(
            process.env.sql_server
          )}`
        );
      } else if (process.env.INSTANCE_ID.startsWith("DEMO_97_SD_")) {
        console.log(
          `Deploying (AWS DEV / DEMO BSC Testnet [additional base type] for SDAX) instance, saving to DB: ${chalk.inverse(
            process.env.sql_server
          )}`
        );
      } else if (process.env.INSTANCE_ID.startsWith("PROD_56_SD_")) {
        console.log(
          `Deploying (AWS PROD / BSC Mainnet [additional base type] for SDAX) instance, saving to DB: ${chalk.inverse(
            process.env.sql_server
          )}`
        );
      } else if (process.env.INSTANCE_ID.startsWith("PROD_137_SD_")) {
        console.log(
          `Deploying (AWS PROD / Matic Mainnet [additional base type] for SDAX) instance, saving to DB: ${chalk.inverse(
            process.env.sql_server
          )}`
        );
        ...
        console.log(
          chalk.red.bold.inverse(
            `Unknown INSTANCE_ID (${process.env.INSTANCE_ID})`
          )
        );
        process.exit(1);
      }
  ...
  console.log(
    chalk.red("process.env.NETWORK_ID".padEnd(30, ".")),
    process.env.NETWORK_ID
  );
  console.log(
    chalk.red("process.env.CONTRACT_TYPE".padEnd(30, ".")),
    process.env.CONTRACT_TYPE
  );
  const contractPrefix =
    (process.env.INSTANCE_ID || "local").padEnd(30, ".") + "_";
  console.log(
    chalk.red("process.env.CONTRACT_PREFIX".padEnd(30, ".")),
  ...
  console.log(
    chalk.red("O.addr(bal)".padEnd(30, ".")),
    web3.utils.fromWei(bal)
  );
  console.log(
    chalk.red("deployer.gasPrice (gwei)".padEnd(30, ".")),
    web3.utils.fromWei(
      deployer.networks[deployer.network].gasPrice.toString(),
      "gwei"
    )
  );

  // test DB connection
  const dbData = await db.GetDeployment(
    3,
    `dummy_contractName`,
    `dummy_contractVer`
  );
  ...
  if (
    ...
    process.env.CUSTODY_TYPE !== "THIRD_PARTY_CUSTODY"
  ) {
    console.warn(
      `Missing or unknown CUSTODY_TYPE ("${process.env.CUSTODY_TYPE}"): set to SELF_CUSTODY or THIRD_PARTY_CUSTODY.`
    );
    process.env.CUSTODY_TYPE = "SELF_CUSTODY";
  }
  ...
  switch (process.env.CONTRACT_TYPE) {
    case "COMMODITY":
      ...
      if (
        !deployer.network.includes("-fork") &&
        process.env.RESTORE_CONTRACT !== "YES"
      ) {
    ...
    case "CASHFLOW_BASE":
      ...
      if (
        process.env.ADD_TYPE__TYPE_NAME === undefined ||
        process.env.ADD_TYPE__TYPE_NAME.length == 0
      ) {
        console.log(
          chalk.red.bold.inverse(
      ...
      if (!deployer.network.includes("-fork")) {
        ...
        const { evs: evsBase } = await CONST.web3_tx(
          "addSecTokenType",
          [
            process.env.ADD_TYPE__TYPE_NAME,
