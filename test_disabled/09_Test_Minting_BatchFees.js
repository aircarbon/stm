...
contract("StMaster", accounts => {
    ...
    it(`minting originator fee - should allow decreasing of collared, uncapped batch fee`, async () => {
        ...
        await stm.mintSecTokenBatch(CONST.tokenType.NATURE, CONST.kt1Carbon, 1, M, origFee1, [], [],   { from: accounts[0] });
        const batchId = await stm.getSecTokenBatchCount.call();

        var origFee2 = { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: 100, fee_percBips: 10, fee_min: 9, fee_max: 0 }
    ...
    it(`minting originator fee - should allow decreasing of batch fee after minting`, async () => {
        ...
        origFee2 = { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: 9, fee_percBips: 9, fee_min: 9, fee_max: 9 }; // set all fields down
        await stm.setOriginatorFeeTokenBatch(batchId, origFee2, { from: accounts[0] });
        const batch = await stm.getSecTokenBatch(batchId);
        assert(batch.originator == M, 'unexpected originator on minted batch');
        ...
        origFee2 = { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: 8, fee_percBips: 9,  fee_min: 9, fee_max: 9 }; // set one field down - fee_fixed
        await stm.setOriginatorFeeTokenBatch(batchId, origFee2, { from: accounts[0] });
        
        origFee2 = { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: 8, fee_percBips: 8,  fee_min: 9, fee_max: 9 }; // set one field down - fee_percBips
        await stm.setOriginatorFeeTokenBatch(batchId, origFee2, { from: accounts[0] });

        origFee2 = { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: 8, fee_percBips: 8,  fee_min: 8, fee_max: 9 }; // set one field down - fee_min
        await stm.setOriginatorFeeTokenBatch(batchId, origFee2, { from: accounts[0] });
        
        origFee2 = { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: 8, fee_percBips: 8,  fee_min: 8, fee_max: 8 }; // set one field down - fee_max
        await stm.setOriginatorFeeTokenBatch(batchId, origFee2, { from: accounts[0] });

        origFee2 = { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: 8, fee_percBips: 8,  fee_min: 8, fee_max: 8 }; // all fields unchanged
        await stm.setOriginatorFeeTokenBatch(batchId, origFee2, { from: accounts[0] });
    });
    ...
    it(`minting originator fee - should not allow increasing of batch fees after minting (fee_fixed)`, async () => {
        ...
        origFee2 = { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: 2, fee_percBips: 10, fee_min: 1, fee_max: 10 };
        ...
        origFee2 = { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: 1, fee_percBips: 11, fee_min: 1, fee_max: 10 };
        try { await stm.setOriginatorFeeTokenBatch(batchId, origFee2, { from: accounts[0] }); assert.fail('expected contract exception'); }
        catch (ex) { assert(ex.reason == 'Bad fee args', `unexpected: ${ex.reason}`); }

        origFee2 = { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: 1, fee_percBips: 10, fee_min: 2, fee_max: 10 };
        try { await stm.setOriginatorFeeTokenBatch(batchId, origFee2, { from: accounts[0] }); assert.fail('expected contract exception'); }
        catch (ex) { assert(ex.reason == 'Bad fee args', `unexpected: ${ex.reason}`); }

        origFee2 = { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: 1, fee_percBips: 10, fee_min: 2, fee_max: 11 };
        try { await stm.setOriginatorFeeTokenBatch(batchId, origFee2, { from: accounts[0] }); assert.fail('expected contract exception'); }
        catch (ex) { assert(ex.reason == 'Bad fee args', `unexpected: ${ex.reason}`); }
    });
    ...
    it(`minting originator fee - should not allow non-owner to edit batch fee after minting`, async () => {
        ...
        const origFee2 = { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: 0, fee_percBips: 10, fee_min: 1, fee_max: 10 };
        try {
    ...
    it(`minting originator fee - should not allow minting batch fee cap < collar`, async () => {
        ...
        try {
            origFee = { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: 1, fee_percBips: 10, fee_min: 10, fee_max: 5 }; // bad cap < collar
            await stm.mintSecTokenBatch(CONST.tokenType.NATURE, CONST.kt1Carbon, 1, M, origFee, [], [],   { from: accounts[0] });
    ...
    it(`minting originator fee - should not allow setting of batch fee cap < collar`, async () => {
        ...
        await stm.mintSecTokenBatch(CONST.tokenType.NATURE, CONST.kt1Carbon, 1, M, origFee, [], [],       { from: accounts[0] });
        const batchId = await stm.getSecTokenBatchCount.call();
        origFee = { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: 1, fee_percBips: 10, fee_min: 9, fee_max: 0 }; // edit down - no cap
        await stm.setOriginatorFeeTokenBatch(batchId, origFee,                                         { from: accounts[0] });
        
        try {
            origFee = { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: 1, fee_percBips: 10, fee_min: 9, fee_max: 8 }; // bad cap < collar
            await stm.setOriginatorFeeTokenBatch(batchId, origFee,                                     { from: accounts[0] });
        } catch (ex) { assert(ex.reason == 'Bad fee args', `unexpected: ${ex.reason}`); return; }
        assert.fail('expected contract exception');        
    });
    ...
    it(`minting originator fee - should not allow setting of batch fee basis points > 10000`, async () => {
        ...
        try {
            origFee = { ccy_mirrorFee: false, ccy_perThousand: 0, fee_fixed: 1, fee_percBips: 10001, fee_min: 10, fee_max: 0 }; // bad basis points
            await stm.setOriginatorFeeTokenBatch(batchId, origFee,                                     { from: accounts[0] });
        } catch (ex) { assert(ex.reason == 'Bad fee args', `unexpected: ${ex.reason}`); return; }
        assert.fail('expected contract exception');        
    });
