...
contract("StMaster", accounts => {
    ...
    it('fees (multi) - apply VCS token fee 1000 BP + 1000 KG fixed on a large (0.5 GT) trade (fee on B)', async () => {
        ...
        assert(ledgerB_VcsKgAfter == Number(ledgerB_VcsKgBefore) - Number(expectedFeeKg) - Number(transferAmountKg), 'unexpected ledger A (fee payer) VCS ST quantity after transfer');
    });
