...
contract("StMaster", accounts => {
    ...
    // ST FEES
    it('fees (percentage) - apply VCS carbon fee 100 BP on a trade (fee on A)', async () => {
    ...
    it('fees (percentage) - apply UNFCCC carbon fee 1 BP (min) on a trade 1000 tons (min lot size) (fee on B)', async () => {
    ...
    it('fees (percentage) - apply large (>1 batch ST size) carbon fee 5000 BP on a trade on a newly added ST type', async () => {
        ...
        // test contract owner has received expected new ST type carbon fee
    ...
    it('fees (percentage) - apply ETH ccy fee 100 BP on a trade (fee on A)', async () => {
        ...
        assert(await stm.globalFee_ccyType_Bps(CONST.ccyType.SGD) == 0, 'unexpected USD percentage fee after setting ETH fee structure');
    ...
    it('fees (percentage) - apply USD ccy fee 1 BP on a trade (fee on B)', async () => {
        ...
        assert(await stm.globalFee_ccyType_Bps(CONST.ccyType.SGD) == usdFeePercBips, 'unexpected USD percentage fee after setting USD fee structure');
    ...
    it('fees (percentage) - should have reasonable gas cost for two-sided USD ccy & UNFCCC ST transfer (fees on both sides)', async () => {
        ...
        assert(await stm.globalFee_ccyType_Bps(CONST.ccyType.SGD) == ccyFeeBips, 'unexpected USD basis points fee after setting USD fee structure');
