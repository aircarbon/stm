...
contract("StMaster", accounts => {
    ...
    it(`fees (percentage) - apply NATURE token fee 100 BP on a trade (fee on A)`, async () => {
        ...
        const setFeeTx = await stm.setFee_TokType(CONST.tokenType.NATURE, CONST.nullAddr, { fee_fixed: 0, fee_percBips: feeBips, fee_min: 0, fee_max: 0 } );
    ...
    it(`fees (percentage) - apply CORSIA token fee 1 BP (min) on a trade 1000 tons (min lot size) (fee on B)`, async () => {
        ...
        const setFeeTx = await stm.setFee_TokType(CONST.tokenType.CORSIA, CONST.nullAddr, { fee_fixed: 0, fee_percBips: feeBips, fee_min: 0, fee_max: 0 } );
    ...
    it(`fees (percentage) - apply large (>1 batch ST size) token fee 5000 BP on a trade on a newly added ST type`, async () => {
        ...
        await stm.mintSecTokenBatch(newTypeId, 1000, 1,                            accounts[global.TaddrNdx + 0], CONST.nullFees, [], [], { from: accounts[0] });
        await stm.mintSecTokenBatch(newTypeId, 1000, 1,                            accounts[global.TaddrNdx + 0], CONST.nullFees, [], [], { from: accounts[0] });
        await stm.mintSecTokenBatch(newTypeId, 1000, 1,                            accounts[global.TaddrNdx + 0], CONST.nullFees, [], [], { from: accounts[0] });
        await stm.fund(CONST.ccyType.ETH,            CONST.oneEth_wei,             accounts[global.TaddrNdx + 1],                         { from: accounts[0] });
    ...
    it(`fees (percentage) - apply USD ccy fee 1 BP on a trade (fee on B)`, async () => {
        ...
        const setFeeTx = await stm.setFee_CcyType(CONST.ccyType.SGD, CONST.nullAddr, { fee_fixed: 0, fee_percBips: usdFeePercBips, fee_min: 0, fee_max: 0 } );
    ...
    it(`fees (percentage) - apply ETH ccy & NATURE ST fee on a 0.5 ST trade (fees on both sides)`, async () => {
        ...
        const setCarbonFeeTx = await stm.setFee_TokType(CONST.tokenType.NATURE, CONST.nullAddr, { fee_fixed: 0, fee_percBips: carbonFeeBps, fee_min: 0, fee_max: 0 } );
    ...
    it(`fees (percentage) - should have reasonable gas cost for two-sided USD ccy & CORSIA ST transfer (fees on both sides)`, async () => {
        ...
        const ccyFeeBips = 100;
        const setCcyFeeTx = await stm.setFee_CcyType(CONST.ccyType.SGD, CONST.nullAddr, { fee_fixed: 0, fee_percBips: ccyFeeBips, fee_min: 0, fee_max: 0 } );
        ...
        const carbonFeeBps = 100;
        const setCarbonFeeTx = await stm.setFee_TokType(CONST.tokenType.CORSIA, CONST.nullAddr, { fee_fixed: 0, fee_percBips: carbonFeeBps, fee_min: 0, fee_max: 0 } );
    ...
    it(`fees (percentage) - should round fees to zero for minimal transfers (ccy & carbon)`, async () => {
        ...
        await stm.setFee_CcyType(CONST.ccyType.SGD, CONST.nullAddr,      { fee_fixed: 0, fee_percBips: ccyFeeBips,   fee_min: 0, fee_max: 0 } );
        await stm.setFee_TokType(CONST.tokenType.CORSIA, CONST.nullAddr, { fee_fixed: 0, fee_percBips: carbonFeeBps, fee_min: 0, fee_max: 0 } );
    ...
    it(`fees (percentage) - should not allow non-owner to set global fee structure (carbon)`, async () => {
        try {
            const tx1 = await stm.setFee_TokType(CONST.tokenType.CORSIA, CONST.nullAddr, { fee_fixed: 0, fee_percBips: 1, fee_min: 0, fee_max: 0 }, { from: accounts[1] });
        } catch (ex) { 
            assert(ex.reason == 'Restricted', `unexpected: ${ex.reason}`);
            return;
        }
    ...
    it(`fees (percentage) - should not allow a transfer with insufficient ccy to cover fees`, async () => {
        ...
        await stm.setFee_TokType(CONST.tokenType.CORSIA, CONST.nullAddr, { fee_fixed: 0, fee_percBips: 0, fee_min: 0, fee_max: 0 } );
    ...
    it(`fees (percentage) - should not allow a transfer with insufficient carbon to cover fees`, async () => {
        ...
        await stm.setFee_TokType(CONST.tokenType.CORSIA, CONST.nullAddr, { fee_fixed: 0, fee_percBips: 1, fee_min: 0, fee_max: 0 } );

        try {
            await transferHelper.transferLedger({ stm, accounts, 
                    ledger_A: accounts[global.TaddrNdx + 0],                                    ledger_B: accounts[global.TaddrNdx + 1],
                       qty_A: 0,                                                             tokenTypeId_A: 0,
                       qty_B: new BN(CONST.ktCarbon),                                        tokenTypeId_B: CONST.tokenType.CORSIA,
                ccy_amount_A: new BN(CONST.millionCcy_cents),                                  ccyTypeId_A: CONST.ccyType.SGD,
                ccy_amount_B: 0,                                                               ccyTypeId_B: 0,
                   applyFees: true,
            });
