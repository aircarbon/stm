...
contract("StMaster", accounts => {
    ...
    it(`token types - should be able to use newly added ST types`, async () => {
        // add new ST type
        const addSecTokenTx = await stm.addSecTokenType('NEW_TYPE_NAME_2', CONST.settlementType.SPOT, 0, 0);
        const types = (await stm.getSecTokenTypes()).tokenTypes;
    ...
    it(`token types - should not allow adding an existing ST type name`, async () => {
        try {
            const types = (await stm.getSecTokenTypes()).tokenTypes;
            await stm.addSecTokenType(types[0].name, CONST.settlementType.SPOT, 0, 0, { from: accounts[0] });
        } catch (ex) { 
