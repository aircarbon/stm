...
contract("StMaster", accounts => {
    ...
    it(`FT types - should be able to add a future on a spot underlyer`, async () => {
        ...
        const addFtTx = await stm.addSecTokenType(`FT_TEST_${new Date().getTime()}`, CONST.settlementType.FUTURE, expirySG.toMillis(), spotTypes[0].id);
    ...
    it(`FT types - should not be able to add a future with an invalid expiry time`, async () => {
        const spotTypes = (await stm.getSecTokenTypes()).tokenTypes.filter(p => p.settlementType == CONST.settlementType.SPOT);
        try {
            const addFtTx = await stm.addSecTokenType(`FT_TEST_${new Date().getTime()}`, CONST.settlementType.FUTURE, 0, spotTypes[0].id);
        }
    ...
    it(`FT types - should not be able to add a future on invalid (non-existent) underlyer`, async () => {
        try {
            const addFtTx = await stm.addSecTokenType(`FT_TEST_${new Date().getTime()}`, CONST.settlementType.FUTURE, DateTime.local().toMillis(), 0);
        }
    ...
    it(`FT types - should not be able to add a future on invalid (non-spot) underlyer`, async () => {
        const spotTypes = (await stm.getSecTokenTypes()).tokenTypes.filter(p => p.settlementType == CONST.settlementType.SPOT);
        const addFtTx_ok = await stm.addSecTokenType(`FT_TEST_${new Date().getTime()}`, CONST.settlementType.FUTURE, DateTime.local().toMillis(), spotTypes[0].id);
        const ftTypes = (await stm.getSecTokenTypes()).tokenTypes.filter(p => p.settlementType == CONST.settlementType.FUTURE);
        try {
            const addFtTx_err = await stm.addSecTokenType(`FT_TEST_${new Date().getTime()}`, CONST.settlementType.FUTURE, DateTime.local().toMillis(), ftTypes[0].id);
        }
