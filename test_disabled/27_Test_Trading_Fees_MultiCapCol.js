...
contract("StMaster", accounts => {
    ...
    it(`fees (multi-capcol) - apply NATURE token fee 1000 BP + 5 KG fixed (cap 10 KG) on a small trade (fee on A)`, async () => {
        ...
        // set fee structure NATURE: 10% + 5 KG, CAP 10 KG
        const feeBps = 1000; 
        const feeFix = 5;
        const feeCap = 10;
        const setEeuFeeTx = await stm.setFee_TokType(CONST.tokenType.NATURE, CONST.nullAddr, { fee_fixed: feeFix, fee_percBips: feeBps, fee_min: 0, fee_max: feeCap } );
        ...
        truffleAssert.eventEmitted(setEeuFeeTx, 'SetFeeTokMax', ev => ev.tokenTypeId == CONST.tokenType.NATURE && ev.fee_token_Max == feeCap && ev.ledgerOwner == CONST.nullAddr);
    ...
    it(`fees (multi-capcol) - apply NATURE token fee 1000 BP + 1000 KG fixed (collar 100m tons), on a large (0.5 GT) trade (fee on B)`, async () => {
        ...
        const setEeuFeeTx = await stm.setFee_TokType(CONST.tokenType.NATURE, CONST.nullAddr, { fee_fixed: feeFix, fee_percBips: feeBps, fee_min: feeMin, fee_max: 0 } );
        const setCcyFeeTx = await stm.setFee_CcyType(CONST.ccyType.ETH, CONST.nullAddr,   CONST.nullFees);
        truffleAssert.eventEmitted(setEeuFeeTx, 'SetFeeTokBps', ev => ev.tokenTypeId == CONST.tokenType.NATURE && ev.fee_token_PercBips == feeBps && ev.ledgerOwner == CONST.nullAddr);
        truffleAssert.eventEmitted(setEeuFeeTx, 'SetFeeTokFix', ev => ev.tokenTypeId == CONST.tokenType.NATURE && ev.fee_tokenQty_Fixed == feeFix && ev.ledgerOwner == CONST.nullAddr);
    ...
    it(`fees (multi-capcol) - should allow a capped transfer with otherwise insufficient carbon to cover fees (fee on A)`, async () => {
        ...
        await stm.setFee_TokType(CONST.tokenType.NATURE, CONST.nullAddr, { fee_fixed: feeFix, fee_percBips: feeBps, fee_min: 0, fee_max: feeMax } );
    ...
    it(`fees (multi-capcol) - should not allow a transfer with insufficient carbon to cover collared fees (fee on B)`, async () => {
        ...
        const feeFix = 1;
        const feeMin = 101;
        await stm.setFee_TokType(CONST.tokenType.NATURE, CONST.nullAddr, { fee_fixed: feeFix, fee_percBips: feeBps, fee_min: feeMin, fee_max: 0 } );
        await stm.setFee_CcyType(CONST.ccyType.ETH, CONST.nullAddr,   CONST.nullFees);
